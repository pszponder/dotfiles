...
This repository uses **Chezmoi data variables** to configure things like Git identity and machine type during the first setup.
...
Chezmoi will prompt for a few values if they are not already configured.

| Variable       | Description                                                |
| -------------- | ---------------------------------------------------------- |
| `git_name`     | Your Git full name used in `.gitconfig`                    |
| `git_email`    | Your Git email address                                     |
| `machine_type` | Defines whether the machine is a `workstation` or `server` |

These values are stored in Chezmoi's **data configuration** and are used by templates and install scripts.
...
Example usage in script templates:
...
You can view all stored variables with:
...
```
{
  "git_name": "Frodo Baggins",
  "git_email": "frodo@shire.me",
...
You can override variables at runtime using the `--data` flag:

```
chezmoi apply --data machine_type=server
```

Example:

```
chezmoi apply --data git_email=newemail@example.com
```

#### Persistently Changing a Variable

To permanently change a stored value:

```
chezmoi data set git_email newemail@example.com
```

Example:

```
chezmoi data set machine_type server
```

After updating the value, apply your configuration again:
...
Using Chezmoi data variables allows this dotfiles repository to:

* Support **multiple machines**
* Distinguish **servers vs workstations**
* Avoid hard-coding personal information in templates
* Make the configuration **reproducible and portable**
